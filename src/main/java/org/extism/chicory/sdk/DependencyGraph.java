...
class DependencyGraph {
    ...
    private void checkCollision(String moduleName, String symbol) {
        ...
        } else if (this.registeredSymbols.containsKey(moduleName) && this.registeredSymbols.get(moduleName)
                .contains(symbol)) {
            throw new ExtismException(
                    "Collision detected: a symbol with the given name already exists: " + moduleName + "." + symbol);
        }
    }
    ...
    public boolean validate() {
        ...
        for (var kv : modules.entrySet()) {
            ...
            for (int i = 0; i < imports.importCount(); i++) {
                ...
                if (!registeredSymbols.containsKey(moduleName) || !registeredSymbols.get(moduleName)
                        .contains(symbolName)) {
        ...
        while (!unresolved.isEmpty()) {
            ...
            if (!requiredModules.isEmpty()) {
                ...
                for (String requiredModule : requiredModules.keySet()) {
                    ...
                    } else if (!this.instances.containsKey(requiredModule) && !this.hostModules.contains(
                            requiredModule)) {
